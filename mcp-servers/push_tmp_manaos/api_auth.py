...
class APIAuthManager:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.rate_limit_max_clients = max(100, int(self.config.get("rate_limit_max_clients", 10000)))
        self.rate_limit_client_ttl = max(
            self.rate_limit_window,
            int(self.config.get("rate_limit_client_ttl", self.rate_limit_window * 3)),
        )
        self.rate_limit_cleanup_interval = max(
            1,
            int(self.config.get("rate_limit_cleanup_interval", 200)),
        )
    ...
    def _cleanup_rate_limit_state(self, current_time: float) -> None:
        stale_before = current_time - self.rate_limit_client_ttl
        stale_clients = [
            client_id
    ...
    def _trim_client_count_if_needed(self) -> None:
        if len(self.client_last_seen) <= self.rate_limit_max_clients:
            return

        overflow = len(self.client_last_seen) - self.rate_limit_max_clients
        oldest_clients = sorted(self.client_last_seen.items(), key=lambda item: item[1])

        for client_id, _ in oldest_clients[:overflow]:
            self.client_last_seen.pop(client_id, None)
            self.request_history.pop(client_id, None)
