# n8n APIキーを自動的に設定するスクリプト（既存のAPIキーを使用）

Write-Host "=" * 60
Write-Host "n8n APIキー自動設定スクリプト"
Write-Host "=" * 60
Write-Host ""
...
if (Test-Path $oldConfigPath) {
    try {
        ...
        if ($oldConfig.mcpServers.n8n -and $oldConfig.mcpServers.n8n.env.N8N_API_KEY) {
            ...
            Write-Host "     N8N_BASE_URL: $oldBaseUrl" -ForegroundColor Gray
            Write-Host ""
            Write-Host "  ⚠️  注意: 古い設定ファイルのN8N_BASE_URLは $oldBaseUrl です" -ForegroundColor Yellow
            Write-Host "     新しい設定ファイルでは $baseUrl を使用します" -ForegroundColor Yellow
            Write-Host ""
            Write-Host "  ❓ このAPIキーを使用しますか？ (y/n): " -ForegroundColor Cyan -NoNewline
            $confirm = Read-Host
            if ($confirm -ne "y") {
                Write-Host "  ⚠️  キャンセルされました" -ForegroundColor Yellow
                Write-Host ""
                Write-Host "  新しいAPIキーを入力してください:" -ForegroundColor Cyan
            ...
            Write-Host ""
            Write-Host "  新しいAPIキーを入力してください:" -ForegroundColor Cyan
            Write-Host "  (n8nのWeb UI: $baseUrl → Settings → API → Create API Key)" -ForegroundColor Gray
            Write-Host ""
            $apiKey = Read-Host "APIキー"
            if ([string]::IsNullOrWhiteSpace($apiKey)) {
                Write-Host "[NG] APIキーが入力されていません" -ForegroundColor Red
        ...
        Write-Host "  ⚠️  古い設定ファイルの読み込みエラー: $($_.Exception.Message)" -ForegroundColor Yellow
        Write-Host ""
        Write-Host "  新しいAPIキーを入力してください:" -ForegroundColor Cyan
        $apiKey = Read-Host "APIキー"
        if ([string]::IsNullOrWhiteSpace($apiKey)) {
    ...
    Write-Host "  ⚠️  古い設定ファイルが見つかりません" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "  新しいAPIキーを入力してください:" -ForegroundColor Cyan
    Write-Host "  (n8nのWeb UI: $baseUrl → Settings → API → Create API Key)" -ForegroundColor Gray
    Write-Host ""
...
Write-Host "- ローカルのn8n (localhost:5678) を使用する場合は、設定を変更してください" -ForegroundColor Gray
Write-Host ""
