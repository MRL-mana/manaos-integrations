...
class SQLiteFeedStore:
    ...
    def insert_commentary(self, entry: CommentaryEntry) -> None:
        payload = json.dumps(entry.metadata or {})
        with self._lock, self._connection:
            self._connection.execute(
                """
                INSERT OR REPLACE INTO commentary (
                  id, ts, channel, level, message, metadata
                ) VALUES (?, ?, ?, ?, ?, ?)
                """,
                (
                    entry.id,
    ...
    def fetch_recent_commentary(self, limit: int) -> List[CommentaryEntry]:
        rows = self._connection.execute(
            "SELECT * FROM commentary ORDER BY ts DESC LIMIT ?",
            (limit,),
        ).fetchall()
        return [self._row_to_commentary(row) for row in rows]
    ...
    def _row_to_decision_log(row: sqlite3.Row) -> DecisionLog:
        task_context = json.loads(
            row["task_context"]) if row["task_context"] else {}
        candidates_raw = json.loads(
            row["candidates"]) if row["candidates"] else []
        selected_raw = json.loads(row["selected"]) if row["selected"] else {}
        trade_offs = json.loads(row["trade_offs"]) if row["trade_offs"] else []
        priority_vector = json.loads(
            row["priority_vector"]) if row["priority_vector"] else {}
        return DecisionLog(
            id=row["id"],
            ts=row["ts"],
            loop_id=row["loop_id"],
    ...
    def _row_to_loop_eval(row: sqlite3.Row) -> LoopEval:
        return LoopEval(
            ...
            outcome_score=json.loads(
                row["outcome_score"]) if row["outcome_score"] else {},
    ...
    def _row_to_future_intent(row: sqlite3.Row) -> FutureIntent:
        return FutureIntent(
            ...
            expected_impact=json.loads(
                row["expected_impact"]) if row["expected_impact"] else {},
            requirements=json.loads(
                row["requirements"]) if row["requirements"] else [],
    ...
    @staticmethod
    def _row_to_commentary(row: sqlite3.Row) -> CommentaryEntry:
