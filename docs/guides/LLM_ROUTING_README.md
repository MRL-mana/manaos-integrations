...
```json
{
  ...
  "routing_endpoint": "http://localhost:9501/api/llm/route",
  ...
  "cursor.llm.customModels": [
    {
      ...
      "baseUrl": "http://localhost:9501/api/llm/route",
      "apiKey": "manaos"
    }
  ]
}
...
- `manaos_llm_routing_api.py` - Flask APIサーバー
- `cursor_llm_routing_config.json` - 設定ファイル
- `test_llm_routing.py` - 統合テスト
- `start_llm_routing_api.ps1` - 起動スクリプト
