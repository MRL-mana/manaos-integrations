...
try:
    ...
    logger.warning("⚠️ faster-whisper が利用できません。pip install faster-whisper を実行してください")
...
try:
    ...
    logger.warning("⚠️ webrtcvad が利用できません。pip install webrtcvad を実行してください（VAD改善機能）")
...
class TTSEngine:
    ...
    def _synthesize_voicevox(
        ...
        try:
            ...
            response = requests.post(query_url, params=query_params, timeout=30)
    ...
    def get_speakers(self) -> List[Dict[str, Any]]:
        ...
        if self.active_engine == "voicevox":
            try:
                response = requests.get(f"{self.voicevox_url}/speakers", timeout=10)
                response.raise_for_status()
...
class VoiceConversationLoop:
    ...
    def _audio_capture_loop(self):
        ...
        try:
            # マイク設定
            chunk = 320  # WebRTC VADは10ms, 20ms, 30msのチャンクが必要（320サンプル = 20ms @ 16kHz）
            sample_rate = 16000
            channels = 1
                        ...
                        if speech_buffer:
                            ...
                            if silence_frames < max_silence_frames:
                                ...
                                if len(speech_buffer) > 0:
                                    ...
                                    # 最小音声長チェック
                                    duration = len(complete_audio) / (sample_rate * 2)  # 2バイト/サンプル
