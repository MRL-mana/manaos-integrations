...
Write-Host "=== Reason Enum Ops Snapshot Task Lifecycle Status ===" -ForegroundColor Cyan
...
if ($RequirePass.IsPresent -and -not $pass) {
    Write-Host "[ALERT] ops snapshot task lifecycle latest status is not pass" -ForegroundColor Red
    exit 1
}
