...
class LightweightMonitor:
    ...
    def __init__(self):
        self.data_dir = Path("/root/trinity_workspace/shared/monitoring")
        ...
        try:
            ...
            from cognitive_helper import record_cognitive_event
        ...
        if self.trinity_enabled:
            ...
            for alert in alerts:
                if alert.level == "critical":
                    log_failure('lightweight_monitor', f'{alert.metric}_critical', alert.message)
                    record_cognitive_event('lightweight_monitor', 'critical_alert', {
                        'metric': alert.metric,
                        'value': alert.value,
                        'threshold': alert.threshold
                    }, importance=0.9)
                elif alert.level == "warning":
                    record_cognitive_event('lightweight_monitor', 'warning_alert', {
                        'metric': alert.metric,
                        'value': alert.value
                    }, importance=0.6)
