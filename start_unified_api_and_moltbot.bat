@echo off
REM 統合API (9500) と MoltBot Gateway (8088) をまとめて起動。MCP・秘書ファイル整理を利用するとき用。
cd /d "%~dp0"

echo [1/2] 統合API (9500) を起動します...
start "Unified API 9500" cmd /k "cd /d %~dp0 && python unified_api_server.py"
...
echo.
echo 起動しました。
echo   統合API:     http://127.0.0.1:9500
