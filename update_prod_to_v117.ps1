...
"${Version} - updated $(Get-Date -Format 's')`nBackup: $backup" | Set-Content $readmePath -Encoding UTF8
...
Write-Host "`n[DONE] prod adapter updated to $Version"
Write-Host "       run regression audit next:"
Write-Host "  py -3.10 scripts\run\run_layer2_quick_eval.py --base-model D:\castle_ex_training\castle_ex_v1_1 --output-dir $SrcDir --checkpoint-step 300 --eval-data castle_ex_dataset_layer2_lora_v1_1_6_audit100.jsonl --device-map cuda:0 --reports-dir Reports --max-new-tokens 64"
