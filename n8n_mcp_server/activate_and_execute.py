...
def activate_workflow(workflow_id):
    """ワークフローを有効化"""
    try:
        headers = {"X-N8N-API-KEY": N8N_API_KEY, "Content-Type": "application/json"} if N8N_API_KEY else {"Content-Type": "application/json"}
...
def execute_workflow(workflow_id, payload=None):
    ...
    try:
        ...
        if not webhook_url:
            print(f"[NG] Webhook URLを取得できませんでした")
            return False
