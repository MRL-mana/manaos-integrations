...
2. **接続が追加されているか確認**
   - 一覧に「manaOS統合API」またはURL `http://host.docker.internal:9500` が表示されているか
...
```powershell
# 画像生成APIの確認
curl http://localhost:9500/api/comfyui/generate -X POST -H "Content-Type: application/json" -d '{\"prompt\":\"test\"}'
```
