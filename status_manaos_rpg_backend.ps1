...
if ($payload.listening -and $healthCode -eq "200") {
    $payload.ok_reason = "healthy"
}
elseif ($payload.listening) {
    $payload.ok_reason = "health_not_ok"
}

$pass = ($payload.listening -and $healthCode -eq "200")
...
Write-Host "=== ManaOS RPG Backend Status ===" -ForegroundColor Cyan
...
if ($RequirePass.IsPresent -and -not $pass) {
    Write-Host "[ALERT] RPG backend status is not pass" -ForegroundColor Red
    exit 1
}
