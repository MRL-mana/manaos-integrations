param(
    [string]$OutDir = "D:\castle_ex_training\lora_castle_ex_layer2_v1_1_6_posonly",
    [int]$CheckpointStep = 800,
    [int]$PollSec = 60,
...
$monitorLog = Join-Path $logDir "monitor_v116_ck${CheckpointStep}_$timestamp.log"
$stdoutLog  = Join-Path $logDir "layer2_quick_eval_v116_ck${CheckpointStep}_$timestamp.stdout.log"
$stderrLog  = Join-Path $logDir "layer2_quick_eval_v116_ck${CheckpointStep}_$timestamp.stderr.log"
...
"[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] checkpoint found: $ckPath" | Add-Content -Path $monitorLog -Encoding UTF8
...
"[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] starting quick-eval checkpoint=$CheckpointStep" | Add-Content -Path $monitorLog -Encoding UTF8
...
$gatePath = Join-Path $reportsDir "gate_v116_ck${CheckpointStep}_$timestamp.json"
$gate | ConvertTo-Json -Depth 5 | Set-Content -Path $gatePath -Encoding UTF8

"[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] gate=$gatePath acc=$acc passed=$passed" | Add-Content -Path $monitorLog -Encoding UTF8
Write-Host "[gate_v116] acc=$acc contains_hai=$containsHai contains_repeat=$containsRepeat passed=$passed"
Write-Host "[gate_v116] file=$gatePath"
