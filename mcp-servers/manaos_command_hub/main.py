...
import sys
from pathlib import Path
sys.path.insert(0, str(Path.home() / ".mana_memory"))
try:
    from manaos_memory_integration import get_memory_integration
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/home/mana/mana_workspace/manaos_command_hub/logs/hub.log'),
...
# エラーログ専用ハンドラー
error_log_file = f'/home/mana/mana_workspace/manaos_command_hub/logs/error-{datetime.now().strftime("%Y-%m")}.log'
...
_stats_file = Path('/home/mana/mana_workspace/manaos_command_hub/logs/stats.json')
...
def generate_self_review(log_date: str) -> str:
    ...
    # エラーログから失敗を抽出
    error_log_file = Path(
        f"/home/mana/mana_workspace/manaos_command_hub/logs/error-{date_obj.strftime('%Y-%m')}.log")
...
def search_memory(params: MemorySearchParams):
    ...
    try:
        ...
        if result["status"] == "ok":
            ...
            raise HTTPException(status_code=500, detail=result.get("error", "検索に失敗しました"))
