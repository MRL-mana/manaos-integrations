...
Write-Host "=== Reason Enum Ops Quick Verify Gate Task Lifecycle Status ===" -ForegroundColor Cyan
...
if ($RequirePass.IsPresent -and -not $pass) {
    Write-Host "[ALERT] ops quick verify gate task lifecycle latest status is not pass" -ForegroundColor Red
    exit 1
}
