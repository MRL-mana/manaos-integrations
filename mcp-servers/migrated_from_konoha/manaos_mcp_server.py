...
class ManaOSMCPServer:
    def __init__(self):
        ...
        self.manaos_apis = {
            ...
            "realtime_dashboard": "http://127.0.0.1:5555",
            "screen_sharing": "http://127.0.0.1:5008",
            "image_generator": "http://127.0.0.1:5091",
            "trinity_sync": "http://127.0.0.1:5012"
        }
        self.setup_tools()
            ...
            async with httpx.AsyncClient() as client:
                if action == "start":
                    response = await client.post(
                        "http://127.0.0.1:5008/api/start_sharing",
                        json={
                    ...
                    response = await client.post(
                        "http://127.0.0.1:5008/api/stop_sharing",
                        json={"session_id": session_id}
                    )
                elif action == "status":
                    response = await client.get("http://127.0.0.1:5008/api/status")
    ...
    async def image_generate(self, arguments: Dict[str, Any]) -> CallToolResult:
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    "http://127.0.0.1:5091/generate/ai",
                    json={
                        "prompt": prompt,
                        "style": style,
                        "size": size
