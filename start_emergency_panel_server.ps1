...
# ポート9502が使用中か確認
Write-Host "[1/3] ポート9502の確認..." -ForegroundColor Green
$portInUse = Get-NetTCPConnection -LocalPort 9502 -ErrorAction SilentlyContinue
if ($portInUse) {
    Write-Host "  ⚠️  ポート9502は既に使用中です" -ForegroundColor Yellow
    ...
    Write-Host "  ✅ ポート9502は使用可能です" -ForegroundColor Green
}
...
Write-Host "  緊急パネルURL: http://127.0.0.1:9502/emergency" -ForegroundColor Cyan
Write-Host "  緊急パネルURL（Tailscale）: http://100.73.247.100:9502/emergency" -ForegroundColor Cyan
Write-Host ""
