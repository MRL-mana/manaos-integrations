...
class UnifiedOrchestrator:
    ...
    def __init__(
        ...
        if not self.learning_system_url:
            self.learning_system_url = self.config.get("learning_system_url", "http://localhost:5126")
    ...
    async def execute(
        ...
        try:
            ...
            if execution_result and execution_result.get("status") == "completed":
                ...
                result.error = execution_result.get("error") if execution_result else "タスク実行失敗"
                logger.error(f"❌ [{execution_id}] タスク実行失敗")
            
            # 5. 実行結果評価（オプション）
            if (auto_evaluate if auto_evaluate is not None else self.config.get("auto_evaluate", True)):
            ...
            # 6. 記憶に保存（オプション）
            if (save_to_memory if save_to_memory is not None else self.config.get("save_to_memory", True)):
        ...
        try:
            if self.intelligent_retry:
                retry_result = await self.intelligent_retry.execute_with_retry(
                    _call_intent_api,
                    circuit_breaker_key="intent_router"
                )
    ...
    async def _create_plan(self, input_text: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            if self.intelligent_retry:
                retry_result = await self.intelligent_retry.execute_with_retry(
                    _call_planner_api,
                    circuit_breaker_key="task_planner"
                )
                if retry_result.success:
                    result = retry_result.result
                else:
    ...
    async def _wait_for_task_completion(self, task_id: str, timeout: Optional[int] = None) -> Optional[Dict[str, Any]]:
        """タスクの完了を待つ"""
        import time
    ...
    async def _evaluate_result(
        ...
        try:
            if self.intelligent_retry:
                retry_result = await self.intelligent_retry.execute_with_retry(
                    _call_critic_api,
                    circuit_breaker_key="task_critic"
                )
                if retry_result.success:
                    result = retry_result.result
                else:
...
def get_history_endpoint():
    ...
    return jsonify({
        "results": [asdict(r) for r in history],
        "count": len(history)
    })
