...
class StitchMCPServer:
    ...
    async def generate_ui(self, prompt: str, design_type: str = "web") -> dict:
        ...
        try:
            ...
            # プロンプト入力
            # NOTE: 実際のセレクタはStitchのUIに依存
            # ここでは簡易版として、テキストエリアを探す
            prompt_input = None
            for selector in [
                'textarea',
                'input[type="text"]',
            ...
            # 生成完了待機
            await asyncio.sleep(15)  # 生成時間を待つ
...
async def main():
    ...
    try:
        ...
        # サンプル生成をテスト
        result = await server.generate_ui(
            "シンプルなログインフォーム",
            "web"
        )
        logger.info(f"🎨 サンプル生成: {result}")
        
        # 長時間実行する場合は、ここでイベントループを維持
