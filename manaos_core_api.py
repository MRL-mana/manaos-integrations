...
class ManaOSCoreAPI:
    ...
    DANGEROUS_ACTIONS = {
        ...
        "database_drop": {"description": "データベース削除", "blocked": True},
        "network_request": {
    ...
    def remember(self, input_data: Dict[str, Any], format_type: str = "auto"):
        ...
        logger.info(
            f"[Remember] {format_type}: {input_data.get('content', str(input_data))[:50]}..."
        )
        return memory_entry
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if isinstance(mrl_results, dict) and "results" in mrl_results:
                        ...
                        logger.debug(
                            f"[MRL Memory] 検索成功: {len(mrl_results.get('results', []))}件"
                        )
                elif response.status_code == 401:
                    # 認証エラーの場合、APIキーが未設定の可能性
                    logger.debug(f"[MRL Memory] 認証エラー（APIキー未設定の可能性）: HTTP 401")
    ...
    def _check_safety(self, action_type: str, args: Dict[str, Any]) -> tuple[bool, Optional[str]]:
        ...
        if action_type in self.DANGEROUS_ACTIONS:
            ...
            if danger_config.get("blocked", False):
                return (
                    False,
                    f"危険な操作がブロックされました: {danger_config['description']} ({action_type})",
                )
            ...
            if action_type == "file_delete":
                ...
                if allowed_commands and command not in allowed_commands:
                    return (
                        False,
                        f"システムコマンドがブロックされました: {command} (許可されていないコマンド)",
                    )
                ...
                if allowed_domains:
                    ...
                    if domain not in allowed_domains:
                        return (
                            False,
                            f"ネットワークリクエストがブロックされました: {url} (許可されていないドメイン)",
                        )
                ...
                for blocked_path in blocked_paths:
                    if path.startswith(blocked_path):
                        return (
                            False,
                            f"ファイル書き込みがブロックされました: {path} (ブロックされたパス: {blocked_path})",
                        )
    ...
    def act(self, action_type: str, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if action_type == "lfm25_call" or action_type == "lightweight_llm":
            try:
                ...
                response = client.chat(
                    message=message, model=ModelType.ULTRA_LIGHT, task_type=task_type_enum
                )
        ...
        if action_type == "create_story_video" or action_type == "svi_story":
            try:
                ...
                action["result"] = result
                logger.info(
                    f"[Act] SVIストーリー動画生成: {len(result.get('execution_ids', []))}セグメント"
                )
                return result
            except ImportError:
                logger.error("requestsライブラリが利用できません")
        ...
        if action_type == "search_models" or action_type == "hf_search":
            ...
            try:
                ...
                results = hf.search_models(
                    query=query, task=args.get("task"), limit=args.get("limit", 10)
                )
        ...
        # Stable Diffusion プロンプト生成（Ollama統合・sd-prompt.ps1と同様の正規化・Negative対応）
        if action_type == "generate_sd_prompt" or action_type == "sd_prompt":
            try:
                import requests

                japanese_description = (
                    args.get("prompt", args.get("description", "")) or ""
                ).strip()
                ...
                request_body = {
                    ...
                    "stream": False,
                    "options": {"temperature": temperature, "top_p": 0.95, "top_k": 40},
                }

                response = requests.post(
                    f"{ollama_url}/api/generate", json=request_body, timeout=90
                )
                response.raise_for_status()
                result_data = response.json()

                raw_prompt = (
                    result_data.get("response") or result_data.get("message") or ""
                ).strip()
                if not raw_prompt:
        ...
        if action_type == "brave_search" or action_type == "brave_web_search":
            ...
            try:
                ...
                self.emit(
                    "web_search",
                    {"provider": "brave", "query": query, "count": len(results)},
                    "normal",
                )
                return result
            except Exception as e:
        ...
        if action_type == "base_ai_chat" or action_type == "base_ai_completion":
            ...
            try:
                ...
                response = base_ai.chat_simple(prompt=prompt, system_prompt=system_prompt)

                result = {"response": response, "model": "base-ai", "use_free": use_free}
                ...
                self.emit(
                    "llm_call",
                    {"provider": "base_ai", "prompt": prompt[:100], "use_free": use_free},
                    "normal",
                )
                return result
            except Exception as e:
    ...
    def save_conversation(
        self, user_message: str, assistant_response: str, context: Optional[Dict[str, Any]] = None
    ):
        """
...
def save_conversation(
    user_message: str, assistant_response: str, context: Optional[Dict[str, Any]] = None
):
    """Cursorでの会話を自動的に記憶システムに保存"""
...
if __name__ == "__main__":
    ...
    # LLM呼び出し
    result = manaos.act(
        "llm_call", {"task_type": "conversation", "prompt": "こんにちは、今日はいい天気ですね。"}
    )
