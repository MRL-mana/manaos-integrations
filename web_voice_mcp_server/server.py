"""
Web Voice Interface MCPサーバー
Web音声インターフェース機能をMCPツールとして提供
"""
...
# APIエンドポイント
WEB_VOICE_API_URL = os.getenv("WEB_VOICE_API_URL", "http://localhost:5115")
...
def call_api(endpoint: str, method: str = "GET", data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Web Voice APIを呼び出す"""
    url = f"{WEB_VOICE_API_URL}{endpoint}"
...
if MCP_AVAILABLE:
        ...
        return [
            ...
            Tool(
                ...
                inputSchema={
                    "type": "object",
                    "properties": {
                        "text": {"type": "string", "description": "音声認識テキスト"},
    ...
    async def call_tool(name: str, arguments: Any) -> List[TextContent]:
        ...
        try:
            ...
            if name == "web_voice_health":
                result = call_api("/health")
            elif name == "web_voice_execute":
                result = call_api("/api/voice/execute", "POST", args)
            elif name == "web_voice_text_execute":
                result = call_api("/api/text/execute", "POST", args)
            elif name == "web_voice_status":
