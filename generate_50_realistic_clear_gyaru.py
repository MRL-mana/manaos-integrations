...
# 成功しているモデルのみ使用（realisticVisionV60B1_v51HyperVAE.safetensorsはメタデータエラーで失敗）
models = [
...
print("=" * 60)
print("マナ好みのリアル系清楚系ギャル50枚生成")
print("（美しさ・魅力・引き締まった体・Dカップ・性行為）")
print("=" * 60)
print()
print("使用モデル: realisian_v60.safetensors（確実に動作するモデル）")
print()

# マナ好みの清楚系ギャル要素（美しさと魅力を強調）
...
poses = ["standing", "sitting", "lying down", "cowgirl position", "missionary position", "doggy style", "69 position", "reverse cowgirl"]
...
for i in range(50):
    ...
    # マナ好みのリアル系清楚系ギャルプロンプト（美しさ・魅力・身体崩れ防止・引き締まった体・Dカップ・性行為）
    prompt_parts = [
        ...
        random.choice(scenes),  # シーン
        random.choice(sex_acts),  # 性行為を明確に追加
...
if len(prompt_ids) > 0:
    print()
    print("プロンプトID一覧（最初の5件と最後の5件）:")
    for i, prompt_id in enumerate(prompt_ids, 1):
        if i <= 5 or i > len(prompt_ids) - 5:
    ...
    print()
    print("生成状況の確認:")
    print("  python check_50_generations.py")
