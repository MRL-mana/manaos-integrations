...
def main():
    ...
    # デグラデッドモード明示（API未起動等で一部失敗時）
    if recorded < len(learnings) or rag_added < len(learnings):
        print(
            f"\n⚠️  Degraded: Learning/RAGの一部が応答しませんでした。レポートは保存済み。次回実行で再試行します。"
        )
