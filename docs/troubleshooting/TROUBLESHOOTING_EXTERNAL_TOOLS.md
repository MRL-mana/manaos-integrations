...
2. **接続が追加されているか確認**
   - 一覧に「manaOS統合API」またはURL `http://host.docker.internal:9510` が表示されているか
...
```powershell
# 画像生成APIの確認
curl http://127.0.0.1:9510/api/comfyui/generate -X POST -H "Content-Type: application/json" -d '{\"prompt\":\"test\"}'
```
