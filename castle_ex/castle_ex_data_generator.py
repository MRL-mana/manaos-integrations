...
class CastleEXDataGenerator:
    ...
    def __init__(self, random_seed: int = 42):
        ...
        self.slots = {
            # 強差分（必須）
            "time": ["06:10", "07:25", "08:40", "09:15", "10:30", "11:45", "12:20", "13:35", "14:50", "15:05", "16:20", "17:35", "18:50", "19:15", "20:30", "21:45", "22:10", "23:25"],
            ...
            # 文脈（ManaOS）
            "place": ["レジ前", "洗車機前", "ピット", "電話対応", "給油機前", "店舗内", "駐車場", "オイル交換エリア"],
            "situation": ["混雑", "人手不足", "雨", "夜間", "連休", "平日", "朝ラッシュ", "夕方ラッシュ", "深夜", "早朝"],
            "task": ["給油", "洗車", "タイヤ空気圧", "オイル交換", "クレーム対応", "会計", "商品説明", "誘導", "清掃"],
            "role": ["新人", "ベテラン", "常連客", "初来店", "高齢者", "家族連れ", "ビジネスマン", "学生"],
            
            # 感情
            "emotion": ["怒り", "不安", "焦り", "安心", "嬉しい", "落ち込み", "困惑", "緊張", "疲れ", "満足"],
            "intensity": ["小", "中", "大"],
            
            # オブジェクト（Layer 0-2用）
            "obj": ["犬", "猫", "車", "セルフ", "レギュラー", "😊", "😢", "🔧", "夜勤", "給油", "洗車", "オイル", "タイヤ", "エンジン", "ブレーキ"],
            "other_obj": ["猫", "犬", "バイク", "ハイオク", "洗車", "😢", "😊", "🔨", "日勤", "洗車", "給油", "エンジン", "タイヤ", "オイル", "ブレーキ"],
            
            # フレーズ（Layer 1用）
            "phrase1": ["怒る", "忙しい", "迷惑", "疲れた", "嬉しい", "悲しい", "不安", "焦る", "困る", "イライラする", "腹が立つ", "手が離せない", "立て込んでる", "やめてほしい"],
            "phrase2": ["イライラする", "手が離せない", "困る", "クタクタ", "幸せ", "つらい", "心配", "急ぐ", "悩む", "腹が立つ", "怒る", "立て込んでる", "忙しい", "迷惑"],
        }
    ...
    def generate_layer_1_data(self, count: int) -> List[Dict]:
        ...
        for _ in range(positive_count):
            ...
            if template_type == "synonym":
                ...
                question = self.fill_slots(template)
                # 反義語のペアを取得
                idx = random.randint(0, min(len(self.slots["phrase1"]), len(self.slots["phrase2"])) - 1)
    ...
    def generate_layer_2_data(self, count: int) -> List[Dict]:
        """Layer 2: 関係層データ生成（スロット化）"""
        data = []
        
        # 分類（スロット化）
        classification_templates = [
            ("{obj} は {category}？", "はい。{obj}は{category}の一種です。"),
        ]
        
        # 部分/全体（スロット化）
        part_whole_templates = [
            ("{part} は {whole} の一部？", "はい。{part}は{whole}に含まれます。"),
        ]
        
        # 属性（スロット化）
        attribute_templates = [
            ("{obj} の {attr} は？", "{attr}は{value}です。"),
        ]
        
        # 比較（スロット化）
        comparison_templates = [
            ("{a} と {b}、どっちが{metric}？", "{a}の方が{metric}です。"),
        ]
        ...
        manaos_relation_templates = [
            ("{task} は {category}？", "はい。/いいえ。"),
            ("{task} は {category} に入る？", "はい。{task}は{category}に含まれます。/いいえ。{task}は{category}ではありません。"),
        ]
        
        # スロット辞書に追加
        if "category" not in self.slots:
            self.slots["category"] = ["動物", "感情", "作業", "文脈", "場所", "時間", "物", "人", "接客", "安全", "整備", "清掃", "事務"]
        if "part" not in self.slots:
            self.slots["part"] = ["タイヤ", "エンジン", "ブレーキ", "オイル", "給油ノズル", "洗車機", "レジ", "ピット"]
        ...
        for _ in range(count):
            ...
            # attributeの場合の特別処理
            if template_type == "attribute":
                attr = random.choice(self.slots["attr"])
                value = random.choice(self.slots["value"])
                question = question.replace("{attr}", attr).replace("{value}", value)
                answer = answer.replace("{attr}", attr).replace("{value}", value)
            # correspondenceの場合の特別処理
            elif template_type == "correspondence":
                role = random.choice(self.slots["role"])
                task = random.choice(self.slots["task"])
                question = question.replace("{role}", role).replace("{task}", task)
                answer = answer.replace("{role}", role).replace("{task}", task)
    ...
    def generate_layer_3_data(self, count: int) -> List[Dict]:
        ...
        emotion_recognition_templates = [
            ("今{emotion}。どう声かけすればいい？", "共感→状況確認→一手提案。{emotion}への配慮が必要。"),
            ("{role}が{emotion}している。対応は？", "共感し、状況を確認。{emotion}への配慮が必要。"),
        ]
        ...
        all_templates = emotion_recognition_templates + emotion_classification_templates + emotion_transition_templates
        ...
        for _ in range(count):
            ...
            axis_evidence = {
                "emotion": [f"{emotion}への共感が必要", "感情の識別・分類・遷移"]
            }
    ...
    def generate_layer_4_data(self, count: int) -> List[Dict]:
        """Layer 4: 文脈基礎層データ生成（axis_evidence追加）"""
        data = []
        
        # 関係性文脈（スロット化、axis_evidence付き）
        relationship_context_templates = [
            ("「やばい」って言われた。{role}から、{situation}だとどういう意味？", "良い/悪いの可能性説明。{role}と{situation}で意味が変わる。"),
        ]
        
        # 状況文脈（スロット化、axis_evidence付き）
        situation_context_templates = [
            ("「大丈夫」({place}で、{role}から) → 意味は？", "強がりの可能性。本当は不安や痛みがあるかもしれない。"),
            ("沈黙（{situation}中）→ 意味は？", "同意、反対、または考え中。追加情報が必要。"),
        ]
        
        # 非言語文脈（スロット化、axis_evidence付き）
        non_verbal_context_templates = [
            ("「いいよ」({emotion}で) → 真意は？", "本当に良い。同意している。"),
        ]
        
        all_templates = relationship_context_templates + situation_context_templates + non_verbal_context_templates
        ...
        for _ in range(count):
            ...
            axis_evidence = {
                "context": [f"{place}という文脈での解釈", f"{role}という関係性", f"{situation}という状況"]
            }
    ...
    def generate_layer_5_data(self, count: int) -> List[Dict]:
        ...
        emotional_causal_negative = [
            ...
            {
                "state0": {"context": "{time}、{situation}、感情:{emotion}", "emotion": "{emotion}"},
                "action": "反応",
                "state1": {"context": "誤った反応", "emotion": "無視"},
            ...
            {
                "state0": {"context": "{time}、{situation}、状況判断が必要", "emotion": "{emotion}"},
                "action": "判断",
                "state1": {"context": "誤った判断", "emotion": "混乱"},
        ...
        # 正例生成（スロット化テンプレートを使用）
        all_positive_templates = emotional_causal_positive_templates + integrated_causal_positive_templates
        ...
        for _ in range(positive_count):
            ...
            item = {
                ...
                "axes": template["axes"],
                "positive": True,
                "state0": {"context": state0_str.split("、")[0] if "、" in state0_str else state0_str, "emotion": emotion},
                "action": template["action"],
                "state1": {"context": state1_str.split("、")[0] if "、" in state1_str else state1_str, "emotion": "自信" if "自信" in state1_str else "安心"},
                "axis_evidence": axis_evidence,
                "messages": [
                    {"role": "user", "content": question},
                    {"role": "assistant", "content": answer}
        ...
        for error_type in self.error_types:
            ...
            while error_type_generated[error_type] < target_count and retry_count < max_retries:
                ...
                if template.get("is_slot_template", False):
                    ...
                    state1_str = self.fill_slots(template["state1"]["context"]) if "state1" in template else "誤った反応"
                    question = self.fill_slots(template["question"])
                    answer = template["answer"]  # 固定の誤回答
                    
                    emotion = random.choice(self.slots["emotion"])
                    axis_evidence = {
                        "logic": ["理由なし（結論のみ）"] if error_type == "missing_reason" else ["原因→結果の説明が必要"],
                        "emotion": [f"{emotion}→励まし/共感が必要（しかし誤った反応）"] if error_type == "emotion_mismatch" else [f"{emotion}→励まし/共感が必要"]
                    }
                    ...
                    item = {
                        ...
                        "state0": {"context": state0_str.split("、")[0] if "、" in state0_str else state0_str, "emotion": emotion},
                        "action": template["action"],
                        "state1": {"context": state1_str, "emotion": "無視" if error_type == "emotion_mismatch" else emotion},
                        "axis_evidence": axis_evidence,
                        "messages": [
                            {"role": "user", "content": question},
                            {"role": "assistant", "content": answer}
            ...
            if error_type_generated[error_type] < target_count:
                print(f"[WARN] {error_type}: 目標{target_count}件に対して{error_type_generated[error_type]}件のみ生成（不足: {target_count - error_type_generated[error_type]}件）")
    ...
    def generate_layer_6_data(self, count: int) -> List[Dict]:
        """Layer 6: 統合層データ生成（3軸完全統合）"""
        data = []
        
        # 複合状況分析（スロット化）
        complex_analysis_templates = [
            ("{time}、{place}で{role}が{task}中。客が{emotion}。どう対応？", 
             "共感（{emotion}）→優先順位（{task}）→現場制約（{place}）を考慮。解決策{n}個提示。対話{minutes}分継続。"),
        ]
        
        # 多者間ダイナミクス（スロット化）
        multi_agent_dynamics_templates = [
            ("{role}({emotion})が{task}中、{role}が見ている → 行動予測は？", 
             "介入を避ける可能性が高い。{emotion}があり対立に巻き込まれたくない。沈黙または話題変更を試みるかも。"),
        ]
        
        # 反事実的推論（スロット化）
        counterfactual_reasoning_templates = [
            ("もし{time}、{emotion}せずに冷静に対応していたら → 結果は？", 
             "対話が{minutes}分継続し、建設的な解決に至った可能性。感情的反応が選択肢を狭めた。"),
        ]
        
        # メタ認知的判断（スロット化）
        meta_cognitive_templates = [
            ("{situation}で判断するには情報が足りない → 適切な対応は？", 
             "追加情報を収集する。推測で行動せず、確認を取る。不確実性を認めることが正しい判断。"),
            ("{place}で{role}の言葉と非言語が矛盾している → 適切な対応は？", 
             "非言語を優先しつつ、直接確認を取る。「{emotion}に見えるけど、本当に大丈夫？」など。"),
        ]
        
        all_templates = complex_analysis_templates + multi_agent_dynamics_templates + counterfactual_reasoning_templates + meta_cognitive_templates
        ...
        layer6_hard_negative_count = max(1, int(layer6_negative_count * 0.05 * 0.8))  # hard negativeの80%
        ...
        for _ in range(normal_count):
            ...
            data.append({
                ...
                "action": "適切な対応",
                "state1": {"context": f"解決策{n}個提示、対話{minutes}分継続", "emotion": "安心"},
                "axis_evidence": {
        ...
        # context_miss
        context_miss_templates = [t for t in layer6_hard_negative if t["error_type"] == "context_miss"]
        for _ in range(context_miss_count):
            template = random.choice(context_miss_templates if context_miss_templates else layer6_hard_negative)
            data.append({
                "layer": 6,
                "axes": ["logic", "emotion", "context"],
                "positive": False,
        ...
        # overconfident
        overconfident_templates = [t for t in layer6_hard_negative if t["error_type"] == "overconfident"]
        for _ in range(overconfident_count):
            template = random.choice(overconfident_templates if overconfident_templates else layer6_hard_negative)
            data.append({
                "layer": 6,
                "axes": ["logic", "emotion", "context"],
                "positive": False,
    ...
    def deduplicate_data(self, all_data: List[Dict]) -> List[Dict]:
        ...
        if duplicate_count > 0:
            print(f"  重複除去: {duplicate_count}件を削除（重複率: {duplicate_ratio:.1%}、残り: {len(unique_data)}件）")
    ...
    def generate_dataset(self, total_count: int, output_path: str, target_unique: Optional[int] = None, 
                         existing_data: Optional[List[Dict]] = None, 
                         target_negative_ratio: Optional[float] = None,
                         target_layer2_count: Optional[int] = None,
                         target_error_type_min_max_ratio: Optional[float] = None) -> Dict[str, Any]:
        ...
        if existing_data:
            ...
            print(f"既存: positive={existing_positive}, negative={existing_negative} ({existing_negative_ratio:.1%})")
            ...
            if target_negative_ratio:
                ...
                print(f"目標負例率: {target_negative_ratio:.1%} → 追加負例必要: {additional_negative_needed}件")
        else:
            all_data = []
        ...
        if target_layer2_count:
            existing_layer2 = sum(1 for item in all_data if item.get("layer") == 2) if existing_data else 0
            layer2_needed = max(0, target_layer2_count - existing_layer2)
            layer_counts[2] = layer2_needed
            print(f"Layer 2: 既存{existing_layer2}件 → 目標{target_layer2_count}件（追加必要: {layer2_needed}件）")
            if layer2_needed > 0:
        ...
        if target_negative_ratio and existing_data:
            ...
            if additional_negative_needed > 0:
                ...
                while len(new_negative) < additional_negative_needed and negative_generation_iterations < max_negative_iterations:
                    negative_generation_iterations += 1
                    # Layer 5の負例を生成（必要数の3倍を生成して重複除去）
                    batch_size = min((additional_negative_needed - len(new_negative)) * 3, remaining_count * 2)
                    if batch_size <= 0:
                        break
                    
                    negative_batch = self.generate_layer_5_data(batch_size)
                    negative_batch = [item for item in negative_batch if not item.get("positive", True)]
                ...
                print(f"  負例追加: {len(new_negative)}件（累計負例: {current_negative}件、負例率: {current_ratio:.1%}、目標: {target_total_negative}件）")
        ...
        if layer2_target > 0:
            ...
            if layer2_needed > 0:
                ...
                while layer2_generated < layer2_needed and layer2_retry_count < layer2_max_retries:
                    ...
                    # Layer 2のデータを生成
                    layer2_batch = self.generate_layer_2_data(min(layer2_needed - layer2_generated + 10, layer2_needed * 2))
                ...
                if layer2_generated < layer2_needed:
                    print(f"  [WARN] Layer 2: 目標に届きませんでした（生成: {layer2_generated}件、不足: {layer2_needed - layer2_generated}件）")
        ...
        if target_error_type_min_max_ratio and target_error_type_min_max_ratio > 0:
            ...
            if error_type_counts:
                ...
                print(f"  現在: 最小={min_count}件、最大={max_count}件、最小/最大={current_ratio:.2f}")
                
                if current_ratio < target_error_type_min_max_ratio:
                    # 目標最小数を計算
                    target_min = int(max_count * target_error_type_min_max_ratio)
                    print(f"  目標最小数: {target_min}件（最大{max_count}件の{target_error_type_min_max_ratio:.1%}）")
                    ...
                    for error_type, current_count in error_type_counts.items():
                        if current_count < target_min:
                            ...
                            print(f"  {error_type}: {current_count}件 → {target_min}件（+{needed}件必要）")
                            ...
                            while generated < needed and retry_count < max_retries:
                                ...
                                batch = [item for item in batch if not item.get("positive", True) and item.get("error_type") == error_type]
                                
                                if not batch:
                                    # 該当error_typeが生成されなかった場合は、さらに大きなバッチで再試行
                                    if retry_count % 10 == 0:
                                        print(f"    [リトライ {retry_count}] {error_type}が見つかりません。より大きなバッチで再試行...")
                                    continue
                                ...
                                # 進捗表示（10回ごと）
                                if retry_count % 10 == 0:
                                    print(f"    [進捗] {generated}/{needed}件生成済み（リトライ: {retry_count}回）")
                    ...
                    if additional_negative:
                        ...
                        for item in all_data:
                            if not item.get("positive", True):
                                error_type = item.get("error_type")
                                if error_type:
                                    error_type_counts_after[error_type] = error_type_counts_after.get(error_type, 0) + 1
                        ...
                        if error_type_counts_after:
                            ...
                            print(f"  調整後: 最小={min_after}件、最大={max_after}件、最小/最大={ratio_after:.2f}")
        ...
        stats = {
            "total": len(all_data),
            "by_layer": {layer: len([d for d in all_data if d["layer"] == layer]) 
...
def main():
    ...
    parser.add_argument('--count', type=int, default=2000, help='生成するデータ数（目標、デフォルト: 2000）')
    parser.add_argument('--target-unique', type=int, default=None, help='目標ユニーク数（指定しない場合はcount*0.75）')
    parser.add_argument('--existing', type=str, default=None, help='既存データファイル（追加生成モード）')
    parser.add_argument('--target-negative-ratio', type=float, default=None, help='目標負例率（既存データがある場合に有効）')
    parser.add_argument('--target-layer2-count', type=int, default=None, help='目標Layer2件数（既存データがある場合に有効）')
    parser.add_argument('--target-error-type-ratio', type=float, default=None, help='目標error_type最小/最大比（0.7推奨）')
    parser.add_argument('--output', type=str, default='castle_ex_dataset.jsonl', 
