...
$summary = [pscustomobject][ordered]@{
    ...
    status_latest_failure_category = if (-not [string]::IsNullOrWhiteSpace([string]$payload.latest_failure_category)) { [string]$payload.latest_failure_category } elseif (-not [string]::IsNullOrWhiteSpace([string]$payload.opsWatch.latestFailureCategory)) { [string]$payload.opsWatch.latestFailureCategory } else { '' }
    status_latest_failure_notify_attempted = if ($null -ne $payload.latest_failure_notify_attempted) { $payload.latest_failure_notify_attempted } elseif ($null -ne $payload.opsWatch.latestFailureNotifyAttempted) { $payload.opsWatch.latestFailureNotifyAttempted } else { $false }
    status_latest_failure_notified = if ($null -ne $payload.latest_failure_notified) { $payload.latest_failure_notified } elseif ($null -ne $payload.opsWatch.latestFailureNotified) { $payload.opsWatch.latestFailureNotified } else { $false }
    status_latest_failure_notify_suppressed_reason = if (-not [string]::IsNullOrWhiteSpace([string]$payload.latest_failure_notify_suppressed_reason)) { [string]$payload.latest_failure_notify_suppressed_reason } elseif (-not [string]::IsNullOrWhiteSpace([string]$payload.opsWatch.latestFailureNotifySuppressedReason)) { [string]$payload.opsWatch.latestFailureNotifySuppressedReason } else { '' }
    status_latest_degraded_notify_attempted = if ($null -ne $payload.latest_degraded_notify_attempted) { $payload.latest_degraded_notify_attempted } elseif ($null -ne $payload.opsWatch.latestDegradedNotifyAttempted) { $payload.opsWatch.latestDegradedNotifyAttempted } else { $false }
    status_latest_degraded_notified = if ($null -ne $payload.latest_degraded_notified) { $payload.latest_degraded_notified } elseif ($null -ne $payload.opsWatch.latestDegradedNotified) { $payload.opsWatch.latestDegradedNotified } else { $false }
    status_latest_degraded_notify_suppressed_reason = if (-not [string]::IsNullOrWhiteSpace([string]$payload.latest_degraded_notify_suppressed_reason)) { [string]$payload.latest_degraded_notify_suppressed_reason } elseif (-not [string]::IsNullOrWhiteSpace([string]$payload.opsWatch.latestDegradedNotifySuppressedReason)) { [string]$payload.opsWatch.latestDegradedNotifySuppressedReason } else { '' }
