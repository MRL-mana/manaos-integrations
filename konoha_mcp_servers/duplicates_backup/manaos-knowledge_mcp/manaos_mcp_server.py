...
class ManaOSMCPServer:
    def __init__(self):
        ...
        try:
            import sys
            sys.path.insert(0, '/root/trinity_workspace/systems')
            from production_integration import production_integration
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                ...
                "/root/trinity_workspace/mcp/image_editor_mcp_server.py"
            )
...
詳細: {json.dumps(status_result, indent=2, ensure_ascii=False)}"""
    ...
    async def copilot_cli_execute(self, arguments: Dict[str, Any]) -> CallToolResult:
        ...
        try:
            ...
            bridge_script = "/root/trinity_workspace/scripts/copilot_trinity_bridge.sh"
            ...
            result = subprocess.run(
                ...
                cwd="/root/trinity_workspace/scripts"
            )
    ...
    async def x280_rdp_connect(self, arguments: Dict[str, Any]) -> CallToolResult:
        ...
        try:
            ...
            if auto:
                ...
                # 手動接続モード: コマンドを返す
                cmd = f"export DISPLAY=:99 && xfreerdp /v:100.127.121.20 /u:mana /cert:ignore /f /bpp:24"
                if password:
                    cmd += f" /p:{password}"
