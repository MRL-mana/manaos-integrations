...
# 統合APIサーバーをバックグラウンドで起動（PORT=9502）
$serverProcess = Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$PSScriptRoot'; `$env:PYTHONIOENCODING='utf-8'; `$env:PORT='9502'; py -3.10 unified_api_server.py" -PassThru -WindowStyle Minimized
...
try {
    Invoke-RestMethod -Uri "http://127.0.0.1:9502/health" -TimeoutSec 5 | Out-Null
    if ($true) {
