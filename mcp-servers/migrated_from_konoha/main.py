...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/root/manaos_command_hub/logs/hub.log'),
...
# エラーログ専用ハンドラー
error_log_file = f'/root/manaos_command_hub/logs/error-{datetime.now().strftime("%Y-%m")}.log'
...
_stats_file = Path('/root/manaos_command_hub/logs/stats.json')
...
def api_status():
    ...
    today_failures = _stats.get(f"failure_{today}", 0)
    today_total = today_success + today_failures
    success_rate = (today_success / today_total * 100) if today_total > 0 else 100.0
    result["command_hub"] = {
        "status": "healthy",
        "stats": {
            "today": {
...
@app.get("/status")
def status():
    # MCP tool endpoint: same as /api/status
    return api_status()
