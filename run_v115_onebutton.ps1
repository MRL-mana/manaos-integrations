...
$stdoutLog = Join-Path $logDir "layer2_lora_v115_train_${timestamp}.stdout.log"
$stderrLog = Join-Path $logDir "layer2_lora_v115_train_${timestamp}.stderr.log"
$launchLog = Join-Path $logDir "layer2_lora_v115_train_${timestamp}.launch.log"
$monitorScript = Join-Path $root "monitor_v115_ckpt_then_quick_eval.ps1"
...
$outputDir = "D:\castle_ex_training\lora_castle_ex_layer2_v1_1_5_stylefix"
...
$defaultResume = "D:\castle_ex_training\lora_castle_ex_layer2_v1_1_4_stylefix\checkpoint-4500"
...
if (-not $SkipDataGen) {
    ...
    if (Test-Path $generator) {
        Write-Host "[INFO] generating dataset before v1.1.5 training..."
...
if (-not (Test-Path $resumePath)) {
    throw "resume checkpoint not found: $resumePath"
}

$alreadyRunning = Get-CimInstance Win32_Process |
    Where-Object { $_.CommandLine -like "*train_castle_ex_lora.py*" -and $_.CommandLine -like "*v1_1_5_stylefix*" }

if ($alreadyRunning) {
    Write-Host "[INFO] v1.1.5 stylefix training already running."
...
@(
    "[v1.1.5] START style-fix LoRA training",
...
Write-Host "[OK] launched v1.1.5 style-fix training"
