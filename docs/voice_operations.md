...
- デフォルトで `http://localhost:9500` で待ち受け
...
```powershell
# 統合APIが起動している前提
curl http://localhost:9500/api/voice/health
```
...
```powershell
curl http://localhost:9500/api/voice/metrics
```
...
# 音声ファイルで会話まで確認する場合
python scripts\voice\test_voice_e2e.py --base http://localhost:9500 --audio path\to\sample.wav
```
...
| 2 | 統合API起動 | `curl http://localhost:9500/health` が 200 |
| 3 | 音声ヘルス OK | `curl http://localhost:9500/api/voice/health` で `status: ok` |
...
| E2Eテスト | `scripts\voice\run_voice_e2e.bat` または `python scripts/voice/test_voice_e2e.py --base http://localhost:9500` |
