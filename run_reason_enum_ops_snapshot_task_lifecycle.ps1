...
if ([string]::IsNullOrWhiteSpace($LatestJsonFile)) {
    $LatestJsonFile = Join-Path $scriptDir "logs\reason_enum_ops_snapshot_task_lifecycle.latest.json"
}
...
$uninstallScript = Join-Path $scriptDir "uninstall_reason_enum_ops_snapshot_task.ps1"
$statusScript = Join-Path $scriptDir "status_reason_enum_ops_snapshot_task.ps1"
$exportScript = Join-Path $scriptDir "export_reason_enum_ops_snapshot.ps1"

foreach ($required in @($installScript, $uninstallScript, $statusScript, $exportScript)) {
...
$steps.Add((Invoke-Step -Name 'uninstall_pre' -ScriptPath $uninstallScript -Args @('-TaskName', $TaskName)))
$steps.Add((Invoke-Step -Name 'status_expect_missing' -ScriptPath $statusScript -Args @('-TaskName', $TaskName, '-AsJson') -ExpectedExitCodes @(1)))
$steps.Add((Invoke-Step -Name 'export_once' -ScriptPath $exportScript -Args @()))
$steps.Add((Invoke-Step -Name 'install' -ScriptPath $installScript -Args @('-TaskName', $TaskName, '-IntervalMinutes', "$IntervalMinutes")))
$steps.Add((Invoke-Step -Name 'status_after_install_json' -ScriptPath $statusScript -Args @('-TaskName', $TaskName, '-AsJson')))

if (-not $KeepInstalled.IsPresent) {
    $steps.Add((Invoke-Step -Name 'uninstall_post' -ScriptPath $uninstallScript -Args @('-TaskName', $TaskName)))
    $steps.Add((Invoke-Step -Name 'status_after_uninstall_expect_missing' -ScriptPath $statusScript -Args @('-TaskName', $TaskName, '-AsJson') -ExpectedExitCodes @(1)))
}

$failed = @($steps | Where-Object { -not $_.ok })
$ok = ($failed.Count -eq 0)
$okReason = if ($ok) { 'ops_snapshot_task_lifecycle_passed' } else { 'ops_snapshot_task_lifecycle_failed' }
$failedStepNames = @($failed | ForEach-Object { [string]$_.name })
$statusSummary = @(
    $steps | Where-Object { $_.name -like 'status*' } | ForEach-Object {
        @($_.output_tail | Where-Object { $_ -match 'snapshot_ok:' -or $_ -match 'snapshot_ok_reason:' -or $_ -match 'task_found:' -or $_ -match 'task_last_result:' })
    }
)
...
if ($ok) {
    Write-Host "[OK] ops snapshot task lifecycle completed" -ForegroundColor Green
    exit 0
}

Write-Host "[ALERT] ops snapshot task lifecycle failed" -ForegroundColor Red
