...
```powershell
# CivitAI検索テスト
Invoke-RestMethod -Uri "http://localhost:9500/api/civitai/search?query=realistic&limit=3" -Method GET
...
3. ✅ 統合APIサーバーの状態確認
   ```powershell
   Invoke-RestMethod -Uri "http://localhost:9500/api/integrations/status" -Method GET
