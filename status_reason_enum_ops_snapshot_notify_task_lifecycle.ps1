...
if (Test-Path $LatestJsonFile) {
    try {
        ...
        $rawFailedSteps = @()
        if ($null -ne $latest.failed_steps) {
            $rawFailedSteps = @($latest.failed_steps | ForEach-Object { [string]$_ })
        }
        ...
        $payload.failed_steps = if ($rawFailedSteps.Count -gt 0) { $rawFailedSteps } else { @() }
...
Write-Host "=== Reason Enum Ops Snapshot Notify Task Lifecycle Status ===" -ForegroundColor Cyan
...
if ($RequirePass.IsPresent -and -not $pass) {
    Write-Host "[ALERT] ops snapshot notify task lifecycle latest status is not pass" -ForegroundColor Red
    exit 1
}
