...
- **File Secretary**: 統合API に **済** — `GET /api/file-secretary/health`, `GET /api/file-secretary/inbox/status`, `POST /api/file-secretary/files/organize`。`FILE_SECRETARY_URL`（デフォルト http://localhost:5120）で File Secretary を指す。
