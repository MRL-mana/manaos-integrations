...
```bash
# GETリクエスト
curl "http://127.0.0.1:9502/api/brave/search?query=Python&count=10"

# POSTリクエスト
curl -X POST http://127.0.0.1:9502/api/brave/search \
...
```bash
# POSTリクエスト
curl -X POST http://127.0.0.1:9502/api/base-ai/chat \
  -H "Content-Type: application/json" \
  -d '{
    "prompt": "画像生成プロンプトを改善してください: cute girl",
    "use_free": true,
