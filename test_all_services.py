...
def test_intent_router(port: int) -> Dict[str, Any]:
    """Intent Routerテスト"""
    try:
        response = httpx.post(
...
def test_task_planner(port: int) -> Dict[str, Any]:
    """Task Plannerテスト"""
    try:
        # まずヘルスチェック
...
def test_task_critic(port: int) -> Dict[str, Any]:
    """Task Criticテスト"""
    try:
        # まずヘルスチェック
        health_response = httpx.get(f"http://127.0.0.1:{port}/health", timeout=5)
...
def test_rag_memory(port: int) -> Dict[str, Any]:
    """RAG Memoryテスト"""
    try:
        response = httpx.post(
...
def main():
    ...
    tests = [
        ("Intent Router", 5100, test_intent_router),
        ("Task Planner", 5101, test_task_planner),
        ("Task Critic", 5102, test_task_critic),
        ("RAG記憶進化", 5103, test_rag_memory),
        ("汎用タスクキュー", 5104, test_task_queue),
        ("UI操作機能", 5105, test_ui_operations),
        ("統合オーケストレーター", 5106, test_unified_orchestrator),
        ("Executor拡張", 5107, test_executor),
        ("Portal統合", 5108, test_portal_integration),
        ("成果物自動生成", 5109, test_content_generation),
        ("LLM最適化", 5110, test_llm_optimization),
    ]
    
    results = []
    for name, port, test_func in tests:
        result = test_service(name, port, test_func)
