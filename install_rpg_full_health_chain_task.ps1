...
$jobScript = Join-Path $scriptDir "run_rpg_full_health_chain_once.ps1"
...
Write-Host "=== Register RPG Full Health Chain Task ===" -ForegroundColor Cyan
...
$createTask = {
    param([string]$Level)
    $values = @('/Create', '/SC', 'MINUTE', '/MO', "$IntervalMinutes", '/TN', $TaskName, '/TR', $taskRun, '/RL', $Level, '/F')
    if ($useSystemAccount) {
        $values += @('/RU', 'SYSTEM')
    }
    schtasks @values | Out-Null
    return $LASTEXITCODE
}
