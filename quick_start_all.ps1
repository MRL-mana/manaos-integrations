...
if ($apiServerProcess) {
    ...
    Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$scriptDir'; python unified_api_server.py" -WindowStyle Normal
...
Write-Host "  POST http://localhost:9500/api/comfyui/generate - 画像生成" -ForegroundColor Gray
Write-Host "  GET  http://localhost:9500/api/civitai/search - モデル検索" -ForegroundColor Gray
Write-Host "  POST http://localhost:9500/api/google_drive/upload - ファイルアップロード" -ForegroundColor Gray
Write-Host ""
