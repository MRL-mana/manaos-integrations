...
try:
    from manaos_integrations._paths import FILE_SECRETARY_PORT, SLACK_INTEGRATION_PORT
except Exception:  # pragma: no cover
    try:
        from _paths import FILE_SECRETARY_PORT, SLACK_INTEGRATION_PORT  # type: ignore
    except Exception:  # pragma: no cover
...
DEFAULT_SLACK_URL = os.getenv(
    "SLACK_API_URL",
    f"http://127.0.0.1:{SLACK_INTEGRATION_PORT}",
)
DEFAULT_FILE_SECRETARY_URL = os.getenv(
    "FILE_SECRETARY_URL",
...
def check_slack_integration_api():
    ...
    try:
        response = httpx.get(f"{DEFAULT_SLACK_URL}/health", timeout=5.0)
...
def check_file_secretary_api():
    ...
    print("\n=== File Secretary API確認 ===")
    try:
        response = httpx.get(f"{DEFAULT_FILE_SECRETARY_URL}/health", timeout=5.0)
        if response.status_code == 200:
            data = response.json()
...
def check_slack_endpoints():
    ...
    for endpoint, method in endpoints:
        try:
            if method == "GET":
                response = httpx.get(f"{DEFAULT_SLACK_URL}{endpoint}", timeout=2.0)
            else:
                response = httpx.post(
                    f"{DEFAULT_SLACK_URL}{endpoint}",
                    json={},
                    timeout=2.0,
                )
