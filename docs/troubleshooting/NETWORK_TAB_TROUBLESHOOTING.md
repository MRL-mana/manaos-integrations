...
Networkタブで `/api/v1/tools/` や `http://127.0.0.1:9503/openapi.json` へのリクエストが表示されない。
...
- ❌ `http://127.0.0.1:9503/openapi.json` へのリクエスト（見当たらない）
...
2. Tool Server（`http://127.0.0.1:9503`）が登録されているか確認
...
1. **Networkタブでリクエストを確認**
   ...
   - `http://127.0.0.1:9503/openapi.json` へのリクエストがあるか確認
...
- [ ] `http://127.0.0.1:9503/openapi.json` へのリクエストを確認
