# n8nをWindowsサービスとしてインストールするスクリプト（簡易版）
# 管理者権限で実行が必要

param(
    [switch]$SkipNSSMCheck
)
...
if (-not (Test-Path $nssmPath) -and -not $SkipNSSMCheck) {
...
if (-not (Test-Path $nssmPath)) {
    Write-Host "[NG] NSSMが見つかりません: $nssmPath" -ForegroundColor Red
    Write-Host "手動でインストールしてください: https://nssm.cc/download" -ForegroundColor Yellow
    exit 1
}
...
if (-not $service) {
    Write-Host "[NG] サービスが見つかりません" -ForegroundColor Red
    exit 1
}
...
if ($wmiService) {
    Write-Host "起動タイプ: $($wmiService.StartMode)" -ForegroundColor White
}
Write-Host ""
