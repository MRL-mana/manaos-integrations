...
def generate_system3_status(
    ...
    content = (
        ...
        "---\n\n"
        "## Self Assessment\n"
        "- High noise index: Review proposal quality or upper limit control\n"
        "- Low approval rate: Improve proposal granularity and priority\n"
        "- Low execution rate: Check execution issues or post-approval flow\n\n"
    )
...
def update_weekly_review(
    ...
    if score_change > 0.5:
        ...
        reasons_down.append(f"スコアが {abs(score_change):.1f} ポイント下降（{abs(score_change_pct):.1f}%）")
        if todo_metrics.get("expired", 0) > todo_metrics.get("proposed", 0) * 0.5:
            reasons_down.append(f"期限切れタスクが多い: {todo_metrics['expired']:.0f}件")
        if todo_metrics.get("approved", 0) > 0 and todo_metrics.get("executed", 0) / todo_metrics.get("approved", 1) < 0.5:
            reasons_down.append(f"実行率が低い: {todo_metrics['executed']:.0f}/{todo_metrics['approved']:.0f}")
