...
if ([string]::IsNullOrWhiteSpace($LatestJsonFile)) {
    $LatestJsonFile = Join-Path $scriptDir "logs\reason_enum_ops_snapshot_notify_task_lifecycle.latest.json"
}
if ([string]::IsNullOrWhiteSpace($HistoryJsonl)) {
    $HistoryJsonl = Join-Path $scriptDir "logs\reason_enum_ops_snapshot_notify_task_lifecycle.history.jsonl"
}
if ($MaxHistoryLines -lt 1) {
    throw "MaxHistoryLines must be >= 1"
}

$installScript = Join-Path $scriptDir "install_reason_enum_ops_snapshot_notify_task.ps1"
$uninstallScript = Join-Path $scriptDir "uninstall_reason_enum_ops_snapshot_notify_task.ps1"
$statusScript = Join-Path $scriptDir "status_reason_enum_ops_snapshot_notify_task.ps1"
$notifyScript = Join-Path $scriptDir "run_reason_enum_ops_snapshot_notify_once.ps1"

foreach ($required in @($installScript, $uninstallScript, $statusScript, $notifyScript)) {
...
$steps.Add((Invoke-Step -Name 'notify_once' -ScriptPath $notifyScript -StepParameters @()))
$steps.Add((Invoke-Step -Name 'install' -ScriptPath $installScript -StepParameters @('-TaskName', $TaskName, '-IntervalMinutes', "$IntervalMinutes")))
...
$okReason = if ($ok) { 'ops_snapshot_notify_task_lifecycle_passed' } else { 'ops_snapshot_notify_task_lifecycle_failed' }
$failedStepNames = @($failed | ForEach-Object { [string]$_.name })
$statusSummary = @(
    $steps | Where-Object { $_.name -like 'status*' } | ForEach-Object {
        @($_.output_tail | Where-Object { $_ -match 'latest_ok:' -or $_ -match 'latest_ok_reason:' -or $_ -match 'task_found:' -or $_ -match 'task_last_result:' })
    }
)
...
Write-Host "=== Reason Enum Ops Snapshot Notify Task Lifecycle ===" -ForegroundColor Cyan
...
if ($ok) {
    Write-Host "[OK] ops snapshot notify task lifecycle completed" -ForegroundColor Green
    exit 0
}

Write-Host "[ALERT] ops snapshot notify task lifecycle failed" -ForegroundColor Red
